...
if __name__ == '__main__':
    ...
    num_epochs = 200
    best_test_acc = 0.0
    patience = 15  # epochs without val improvement before early stop
    no_improve_epochs = 0
    last_epoch = 0
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        if test_acc > best_test_acc:
            ...
            if no_improve_epochs >= patience:
                print(f"提前停止: 连续 {patience} 个epoch测试集未提升")
                break

    print(f"训练结束，最佳测试准确率: {best_test_acc:.2f}% | 训练轮次: {last_epoch}")
