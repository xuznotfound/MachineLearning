#python train_anime_dcgan.py --batch-size 256 --workers 16 --epochs 50 --sample-interval 200 | tee train1.log
#每一轮都会输出图片和权重
...
def train(args):
    ...
    for epoch in range(args.epochs):
        for i, real in enumerate(dataloader):
            ...
            if total_iters % args.sample_interval == 0:
                with torch.no_grad():
                    fake_samples = netG(fixed_noise).detach().cpu()
                grid = make_grid(fake_samples, padding=2, normalize=True)
                sample_path = output_dir / f"samples_iter_{total_iters:07d}.png"
...
def parse_args():
    parser = argparse.ArgumentParser(description="DCGAN training on Anime Face dataset")
    ...
    parser.add_argument("--sample-interval", type=int, default=250, help="Iterations between sample image dumps")
